...
_run_example() {
    ...
    kubectl --kubeconfig "$kind_consumer" get secret cert-from-consumer -o jsonpath='{.data.tls\.crt}' | base64 -d | openssl x509 -noout -subject \
        | grep -q 'subject=CN=app.internal.corp' || die "Certificate in secret has wrong CN"
    ...
    while !
        kubectl --kubeconfig "$kind_consumer" get secret cert-from-consumer -o jsonpath='{.data.tls\.crt}' | base64 -d | openssl x509 -noout -subject \
            | grep -q 'subject=CN=app.corp.com'
    do
        retry_count=$((retry_count + 1))
        if [[ $retry_count -ge $max_retries ]]; then
            die "Timed out waiting for updated certificate in consumer"
        fi
        sleep 1
    done
